...
export default function Sidebar(props: SidebarProps) {
    ...
    function click_icon(rota: string) {
    ...
    return (
        <>
                ...
                <div className="px-3 py-3 lg:px-5 lg:pl-3">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center justify-start rtl:justify-end">
                            <button onClick={props.click_sidebar} data-drawer-target="logo-sidebar" data-drawer-toggle="logo-sidebar" aria-controls="logo-sidebar" type="button" className="inline-flex items-center p-2 text-sm text-black rounded-lg sm:hidden hover:bg-color-hover focus:outline-none focus:ring-2 focus:ring-color-hover focus:text-white hover:text-color-hover">
                        ...
                        <div className="flex items-center">
                            <div className="flex items-center ms-3">
                                <Link href={'relatorios'} onClick={() => click_icon('/relatorios')} className={`${rota === "/relatorios" ? 'bg-color-hover text-white' : 'text-black'} flex items-center p-2 rounded-lg hover:bg-color-hover hover:text-white group`}>
            ...
            <aside id="logo-sidebar" className="fixed top-0 left-0 z-40 w-64 h-screen pt-20 transition-transform -translate-x-full bg-color-component border-r border-color-border sm:translate-x-0" aria-label="Sidebar">
                <div className="h-full px-3 pb-4 overflow-y-auto bg-color-component">
                    <ul className="space-y-2 font-medium">
                        <li>
                            <Link href={'calendario'} onClick={() => click_icon('/calendario')} className={`${rota === "/calendario" ? 'bg-color-hover text-white' : 'text-black'} flex items-center p-2 rounded-lg hover:bg-color-hover hover:text-white group`}>
                        ...
                        <li>
                            <Link href={'conta'} onClick={() => click_icon('/conta')} className={`${rota === "/conta" ? 'bg-color-hover text-white' : 'text-black'} flex items-center p-2 rounded-lg hover:bg-color-hover hover:text-white group`}>
